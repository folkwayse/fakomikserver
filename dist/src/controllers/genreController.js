import { AllGenres, createNewGenre, getGenreById, patch, MangaByGenreSlug, } from "../models/genre";
export const getMangaByGenreSlug = async (c) => {
    try {
...
export const getGenres = async (c) => {
...
export const createGenre = async (c) => {
    ...
    try {
        const genre = await createNewGenre(name, description, isMature ? true : false);
...
export const getSingleGenre = async (c) => {
    try {
...
export const patchGenre = async (c) => {
    try {
        const id = await c.req.param("id");
        const { name, description, isMature } = await c.req.json();
